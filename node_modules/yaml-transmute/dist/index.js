...
var YAMLValueType;
(function (YAMLValueType) {
    YAMLValueType["OBJECT"] = "object";
    YAMLValueType["ARRAY"] = "array";
    YAMLValueType["PRIMITIVE"] = "primitive";
})(YAMLValueType || (YAMLValueType = {}));
/**
...
function objectType(obj) {
    ...
    if (isPrimitive(obj))
        return YAMLValueType.PRIMITIVE;
...
function objectToPaths(obj) {
    const paths = [];
    function exploreObjectRecursive(currentPath, obj) {
        if (isPrimitive(obj)) {
            if (obj === undefined)
                return;
 ...
 */
function shapeshiftDocument(document, newModel, doc) {
    function shapeshiftRecursive(currentPath, document, newModel) {
        ...
        for (const key of documentKeys) {
            ...
            if (objectType(document[key]) !== objectType(newModel[key])) {
                doc.setIn([...currentPath, key], doc.createNode(emptyObject(objectType(newModel[key]))));
                continue;
            }
 ...
 */
export function parse(yamlContent) {
    ...
    if (!document) {
        throw new Error("Failed to generate YAML context. Multiple YAML documents in the same file are not supported.");
    }
 ...
 */
export function stringify(updatedYaml, context, options) {
    if (!context) {
...
export function mergeContexts(contexts) {
    ...
    function mergeContextsRecursive(to, from) {
        if (yaml.isMap(to) && yaml.isMap(from)) {
            for (const fromPair of from.items) {
                const toPair = to.items.find((toPair) => toPair.key.toJSON() === fromPair.key.toJSON());
                if (!toPair) {
    ...
    const document = contexts[0].document.clone();
