    ...
    result.forEach((classInfo) => {
        ...
        if (Object.keys(classInfo[0]).length > 0) {
            ...
            if (!r && deployDecoratorFound) {
                ...
                let vcpuCount;
                let memoryMb;
                let maxConcurrentRequestsPerInstance;
                let maxConcurrentInstances;
                let cooldownTime;
                let persistent;
                const methods = classInfo[0].methods
                    ...
                    if (methodType === TriggerType.cron && !cronString) {
                        throw new UserError(`Method ${m.name} in class ${classInfo[0].name} has type cron but no cronString specified`);
                    }
                    const usesAuth = m.decorators.find((d) => d.name === "GenezioAuth") !== undefined;
                    return {
                        name: m.name,
                        type: methodType,
                ...
                if (deployDecoratorFound.arguments) {
                    ...
                    if (deployDecoratorFound.arguments["persistent"] === "true") {
                        persistentDecorator = true;
                    }
                    timeout = deployDecoratorFound.arguments["timeout"];
                    storageSize = deployDecoratorFound.arguments["storageSize"];
