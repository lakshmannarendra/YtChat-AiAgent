...
export async function watchPackage(language, projectName, frontends, sdkPath) {
    switch (language) {
        case Language.js:
        case Language.ts:
...
async function watchNodeModules(projectName, frontends, sdkPath) {
    ...
    const linkedFrontends = (await getLinkedFrontendsForProject(projectName)).filter((f) => f.language === Language.ts || f.language === Language.js);
...
async function writeSdkToNodeModules(projectName, frontends, originSdkPath) {
    ...
    const frontendPaths = (frontends || [])
        .map((f) => f.path)
        .concat((await getLinkedFrontendsForProject(projectName))
