...
import { code, debugLogger, doAdaptiveLogAction, logError, setDebuggingLoggerLogLevel, } from "./utils/logging.js";
...
import { createDatabaseCmd, getDatabaseConnectionCmd, listDatabasesCmd, } from "./commands/database.js";
...
program
    .command("analyze")
    .option("--name <name>", "Name of the project you want to analyze. If not set, the project name will be randomly generated.")
    .option("--region <region>", "Region of the project.", "us-east-1")
    .option("--config <config>", "Use a specific `genezio.yaml` to update the detected setup", "./genezio.yaml")
    .option("--format <format>", `Specify the output format at stdout - [json, list, markdown, text]. Default is ${DEFAULT_FORMAT}.`, DEFAULT_FORMAT)
    ...
    .description("Analyze the current directory to determine the infrastructure setup. This command will create a genezio.yaml file in the current directory with the detected setup.")
    .action(async (options) => {
    await analyzeCommand(options).catch(async (error) => {
        logError(error);
        exit(1);
    });
});
program
    ...
    .option("--disable-optimization", "Set this flag to true to disable dependency optimization.", false)
    .option("--image <image>", "Path to the container Dockerfile. If this option is set, it indicates a container-based deployment.", undefined)
    .option("--env <envFile>", "Load environment variables from a given file", undefined)
    .option("--zip <zipPath>", "Deploy a zip file directly. The zip file must contain a valid genezio config file", undefined)
...
program
    .command("local")
    .option("-p, --port <port>", "Set the port for testing your genezio classes locally", String(PORT_LOCAL_ENVIRONMENT))
...
create
    ...
    .addOption(new Option("--region <region>", "Region of the project.").choices(regions.map((region) => region.value)))
    .option("--path <path>", "Path where to create the project.", undefined)
    .option("--default", "Skip the Next.js wizard interactive questions and apply default configuration.", false)
...
This command is useful when the project has dedicated repositories for the backend and the frontend.")
    .action(async (projectName, projectLanguage) => {
...
program
    ...
    .option("--cloudAdapter <cloudAdapter>", "The cloud adapter that will be used.", CloudProviderIdentifier.GENEZIO_CLOUD)
    .option("--output <output>", "The output path of the bundled class.")
    .action(async (options) => {
    await bundleCommand(options).catch(async (error) => {
        logError(error);
...
program
    ...
    await doAdaptiveLogAction(`Cloning project ${options.name}...`, async () => cloneCommand(options.name, options.region, options.stage, path)).catch((error) => {
        logError(error);
        exit(1);
    });
...
program
    .command("database")
    .option("--action <action>", "The action to perform on the database service - list, create, delete.", "list")
