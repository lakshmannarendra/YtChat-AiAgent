...
import { GenezioCloudInputType, } from "../cloudAdapter.js";
...
async function handleBigElementSizeError(element, projectConfiguration, BUNDLE_SIZE_LIMIT = 1.524e9) {
    const size = await getFileSize(element.archivePath);
    if (size > BUNDLE_SIZE_LIMIT) {
        throw new UserError(`Your ${element.type} ${element.name} is too big: ${size} bytes. The maximum size is ${(BUNDLE_SIZE_LIMIT / 1048576).toFixed(2)}MB. Try to reduce the size of your ${element.type}.`);
    }
    if (element.unzippedBundleSize > BUNDLE_SIZE_LIMIT &&
        element.type === GenezioCloudInputType.FUNCTION) {
        throw new UserError(`Your function ${element.name} is too big: ${element.unzippedBundleSize} bytes. The maximum size is ${(BUNDLE_SIZE_LIMIT / 1048576).toFixed(2)}MB. Try to reduce the size of your function.`);
    }
    if (!(element.unzippedBundleSize > BUNDLE_SIZE_LIMIT &&
        element.type === GenezioCloudInputType.CLASS)) {
        debugLogger.debug(`The bundle for ${element.type} ${element.name} is ${(size / 1048576).toFixed(2)}MB out of ${(BUNDLE_SIZE_LIMIT / 1048576).toFixed(2)}MB.`);
        return;
    }
    ...
    if (!dependenciesInfo || !allNonJsFilesPaths) {
        throw new UserError(`Your ${element.type} ${element.name} is too big: ${element.unzippedBundleSize} bytes. The maximum size is ${(BUNDLE_SIZE_LIMIT / 1048576).toFixed(2)}MB. Try to reduce the size of your class.`);
    }
...
export class GenezioCloudAdapter {
    async deploy(input, projectConfiguration, cloudAdapterOptions, stack = [], sourceRepository, environmentVariables) {
        ...
        const promisesDeploy = input.map(async (element) => {
            await handleBigElementSizeError(element, projectConfiguration, element.persistent ? PERSISTENT_BUNDLE_SIZE_LIMIT : BUNDLE_SIZE_LIMIT);
            debugLogger.debug(`Get the presigned URL for ${element.type}: ${element.name} ${element.archiveName}.`);
            const presignedUrl = await getPresignedURL(projectConfiguration.region, element.archiveName ?? "genezioDeploy.zip", projectConfiguration.name, element.name);
            ...
            debugLogger.debug(`Done uploading the content to S3 for ${element.type}: ${element.name}..`);
        });
        ...
        log.info("");
        const response = await deployRequest(projectConfiguration, input, stage, stack, sourceRepository, environmentVariables);
...
function getClassFunctionUrl(baseUrl, cloudProvider, className) {
    if (cloudProvider === CloudProviderIdentifier.GENEZIO_UNIKERNEL ||
