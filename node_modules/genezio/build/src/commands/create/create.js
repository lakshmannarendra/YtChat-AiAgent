...
import { backendTemplates, expressJsTemplate, frontendTemplates, serverlessFunctionJsTemplate, nitroJsTemplate, nuxtTemplate, } from "./templates.js";
...
export async function createCommand(options) {
    ...
    switch (options.type) {
        ...
        case "nextjs": {
            ...
            const yamlIOController = new YamlConfigurationIOController(path.join(projectPath, "genezio.yaml"));
        ...
        case "expressjs": {
            ...
            // Install template packages
            await doAdaptiveLogAction("Installing template dependencies", async () => installTemplatePackages("npm", projectPath));
            // Print success message
...
async function transformVercelToGenezio(projectPath) {
    ...
    if (nativeFs.existsSync(faviconPath)) {
        const genezioFaviconContent = await fetch("https://raw.githubusercontent.com/Genez-io/graphics/main/favicon/genezio.ico");
 ...
 */
async function cloneAndSetupGenezioRepository(repositoryUrl, fs, options) {
 ...
 */
async function recursiveReplace(fs, rootPath, replacements) {
    ...
    if (fromStats.isDirectory()) {
        ...
        const newFileContent = replacements.reduce((acc, [placeholder, value]) => acc.replaceAll(placeholder, value), fileContent);
...
const SUCCESSFULL_CREATE_MONOREPO = (projectPath) => `Project initialized in ${projectPath}.
    ...
    For ${colors.green("testing")} locally, run:
        ...
        genezio local
`;
const SUCCESSFULL_CREATE_MULTIREPO = (projectPath) => `Project initialized in ${projectPath}. Now run:
    ...
    For ${colors.green("testing")} locally, run:
      ...
      Terminal 2 (start the frontend):
        ...
        genezio local
`;
const SUCCESSFULL_CREATE_BACKEND = (projectPath) => `Project initialized in ${projectPath}. Now run:
...
`;
const SUCCESSFULL_CREATE_NEXTJS = (projectPath) => `Project initialized in ${projectPath}. Now run:
    ...
    For ${colors.green("testing")} locally, run:
        ...
        npm run dev
`;
const SUCCESSFULL_CREATE_EXPRESSJS = (projectPath) => `Project initialized in ${projectPath}. Now run:
    ...
    For ${colors.green("testing")} locally, run:
        ...
        genezio local
`;
const SUCCESSFULL_CREATE_NITROJS = (projectPath) => `Project initialized in ${projectPath}. Now run:
    ...
    For ${colors.green("testing")} locally, run:
        ...
        npm run dev
`;
const SUCCESSFULL_CREATE_NUXT = (projectPath) => `Project initialized in ${projectPath}. Now run:
    ...
    For ${colors.green("testing")} locally, run:
        ...
        npm run dev
`;
const SUCCESSFULL_CREATE_SERVERLESS = (projectPath) => `Project initialized in ${projectPath}. Now run:
