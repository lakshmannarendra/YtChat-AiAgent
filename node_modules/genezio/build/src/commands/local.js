...
async function communicateWithProcess(localProcess, unitName, data, processForUnits) {
    try {
        ...
        if (error instanceof AxiosError &&
            ...
            await startLocalUnitProcess(localProcess.startingCommand, localProcess.parameters, unitName, processForUnits, localProcess.envVars, localProcess.type);
 ...
 * @returns
 */
export function retrieveLocalFunctionUrl(functionName, functionType) {
...
export function retrieveLocalSSRUrl(framework) {
    const portEnvKey = `GENEZIO_PORT_${framework.replace(/[^a-zA-Z0-9]/g, "_").toUpperCase()}`;
    if (!process.env[portEnvKey]) {
        throw new UserError(`${portEnvKey} is not exported. We cannot infer the local port for the SSR framework. Please run the command \`${portEnvKey}=<local_port> genezio local\``);
    }
    const port = process.env[portEnvKey];
    return `http://localhost:${port}`;
}
async function getLocalFunctionHttpServerWrapper(entry) {
    return `
...
async function getLocalFunctionHttpServerPythonWrapper(pathString, entry, handler) {
 ...
 */
async function startSsrFramework(ssrConfig, framework, projectConfiguration, stage, port, envFile) {
    ...
    const envConfig = dotenv.config({ path: envPath });
    const loadedEnvVars = envConfig.parsed || {};
    const newEnvObject = await expandEnvironmentVariables(ssrConfig.environment, projectConfiguration, stage, envFile || path.join(ssrConfig.path, ".env"), {
        isLocal: true,
        port: port,
    });
    debugLogger.debug(`Environment variables injected for ${SSRFrameworkName[framework]}:`, JSON.stringify(newEnvObject));
    const portEnvKey = `GENEZIO_PORT_${framework.replace(/[^a-zA-Z0-9]/g, "_").toUpperCase()}`;
    const existingPort = loadedEnvVars[portEnvKey] || process.env[portEnvKey] || newEnvObject[portEnvKey];
    if (framework.toLowerCase() === SSRFrameworkComponentType.nestjs && !existingPort) {
        throw new UserError(`You need to specify the port for Nest.js. You can do this by:
...
3. Adding \`GENEZIO_PORT_NESTJS=<port>\` to your \`.env\` file.
    ...
    try {
        ...
        const spawnCommand = framework === SSRFrameworkComponentType.streamlit
        ...
        childProcess.on("error", (error) => {
            log.error(new Error(`Failed to start ${SSRFrameworkName[framework]} server located in \`${ssrPath}\`: ${error.message}`));
            if (error.stack) {
        ...
        log.error(new Error(`Failed to start ${SSRFrameworkName[framework]} server located in \`${ssrConfig.path}\`: ${errorMessage}`));
    }
}
