...
export async function bundleCommand(options) {
    if ("functionName" in options &&
        ...
        options.handler &&
        options.entry) {
        const functionElement = new FunctionConfiguration(options.functionName, options.functionPath, options.handler, Language.js, options.entry, FunctionType.aws);
    ...
    let cloudProvider = CloudProviderIdentifier.GENEZIO_CLOUD;
    ...
    const sdkResponse = await sdkGeneratorApiHandler([], mapYamlClassToSdkClassConfiguration(backendConfiguration.classes, backendConfiguration.language.name, backendConfiguration.path), backendConfiguration.path, 
    ...
    const projectConfiguration = new ProjectConfiguration(yamlProjectConfiguration, cloudProvider, sdkResponse);
    ...
    if ("className" in options) {
        ...
        const element = projectConfiguration.functions.find((functionInfo) => functionInfo.name == `function-${options.functionName}`);
        if (!element) {
