...
(function (DeleteAction) {
    ...
    DeleteAction["STAGE"] = "stage";
    DeleteAction["CANCEL"] = "cancel";
...
async function askDeleteOptions(projectId, options) {
    let result = { action: DeleteAction.CANCEL };
    // If user simply runs `genezio delete`
    if (!projectId) {
        let action = DeleteAction.STAGE;
        if (!options.stage) {
        ...
        const projectList = await doAdaptiveLogAction("Fetching deployed projects list", listProjects);
        ...
        if (action === DeleteAction.STAGE) {
            ...
            if (!options.stage) {
                ...
                const stageExists = selectedProject.projectEnvs.find((env) => env.name === options.stage);
                if (!stageExists) {
                    throw new UserError(`Stage ${options.stage} does not exist in project ${selectedProject.name} (${selectedProject.id}).`);
                }
            }
            result = {
                action,
...
function deleteOptionsToWarningQuestion(options) {
...
function deleteOprionsToFailMessage(options) {
    switch (options.action) {
        case DeleteAction.PROJECT:
            if (options.project.name) {
