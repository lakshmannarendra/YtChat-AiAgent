...
export async function askCloneOptions(options) {
    const cloneOptions = options ?? {};
    const closestRegionPromise = cloneOptions.region === undefined ? getClosestRegion() : Promise.resolve(undefined);
...
export async function cloneCommand(projectName, region, stage, projectPath) {
    ...
    await downloadProject(url, path.join(tmpFolder, "projectCode.zip")).catch((error) => {
        ...
        throw new Error("Failed to download project. Make sure the project exists and you have access to it.");
    });
