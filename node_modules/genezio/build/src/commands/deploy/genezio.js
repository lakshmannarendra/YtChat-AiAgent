...
import { DASHBOARD_URL, RECOMMENTDED_GENEZIO_TYPES_VERSION_RANGE, REQUIRED_GENEZIO_TYPES_VERSION_RANGE, } from "../../constants.js";
...
import { fileExists, createTemporaryFolder, zipDirectory, isDirectoryEmpty, directoryContainsIndexHtmlFiles, directoryContainsHtmlFiles, deleteFolder, getBundleFolderSizeLimit, zipFile, } from "../../utils/file.js";
...
import { entryFileFunctionMap, FunctionType, Language, } from "../../projectConfiguration/yaml/models.js";
...
import { createBackendEnvVarList, uploadUserCode, evaluateResource, prepareServicesPreBackendDeployment, prepareServicesPostBackendDeployment, excludedFiles, actionDetectedEnvFile, createBackendEnvVarListFromRemote, EnvironmentResourceType, } from "./utils.js";
...
import { packageManagers, PYTHON_DEFAULT_PACKAGE_MANAGER, } from "../../packageManagers/packageManager.js";
import { DEFAULT_PYTHON_RUNTIME, DEFAULT_PYTHON_VERSION_INSTALL, } from "../../models/projectOptions.js";
import { detectPythonVersion } from "../../utils/detectPythonCommand.js";
import { isCI } from "../../utils/process.js";
import { setEnvironmentVariables } from "../../requests/setEnvironmentVariables.js";
export async function genezioDeploy(options) {
    ...
    await prepareServicesPreBackendDeployment(configuration, projectName, options.stage, options.env);
    ...
    backend: if (configuration.backend && !options.frontend) {
        ...
        if (deployClassesResult) {
            if (configuration.backend?.environment) {
                const environmentVariablesFromRemote = await createBackendEnvVarListFromRemote(configuration.backend?.environment, configuration, options.stage || "prod");
                debugLogger.debug(`Environment variables set from remote resources ${JSON.stringify(environmentVariablesFromRemote)}`);
                setEnvironmentVariables(deployClassesResult?.projectId, deployClassesResult?.projectEnvId, environmentVariablesFromRemote);
            }
    ...
    if (configuration.frontend && !options.backend) {
        ...
        if (options.name) {
            ...
            if (!matchingFrontend) {
                const availableFrontends = frontends
                    ...
                    .map((f) => f.name)
                    .join(", ");
                throw new UserError(`No frontend found with name: ${options.name}. Available frontends: ${availableFrontends || "none"}`);
            }
        }
        for (const [index, frontend] of frontends.entries()) {
            ...
            if (options.name) {
                if (!frontend.name) {
                    log.warn(`Frontend #${index + 1} has no name specified in genezio.yaml. Consider adding a name to improve deployment filtering.`);
                    continue;
                }
            ...
            try {
                await doAdaptiveLogAction(`Running frontend ${index + 1} deploy script`, async () => {
                    const newEnvObject = await expandEnvironmentVariables(frontend.environment, configuration, options.stage, 
                    /* envFile */ undefined, {
                        isFrontend: true,
                    });
                    debugLogger.debug(`Environment variables injected for frontend.scripts.deploy:`, JSON.stringify(newEnvObject));
                    return await runScript(frontend.scripts?.deploy, frontend.path || process.cwd(), newEnvObject);
                });
            }
            catch (error) {
                await GenezioTelemetry.sendEvent({
            ...
            const frontendUrl = await deployFrontend(configuration.name, configuration.region, frontend, index, options, configuration).catch(async (error) => {
    ...
    if (configuration.services?.crons) {
        ...
        for (const cron of configuration.services.crons) {
            const yamlFunctionName = await evaluateResource(configuration, [
                EnvironmentResourceType.RemoteResourceReference,
                EnvironmentResourceType.LiteralValue,
            ], cron.function, options.stage || "prod", 
            /* envFile */ undefined, 
            /* options */ undefined);
            if (!deployClassesResult) {
                throw new UserError("Could not deploy cron jobs. Please make sure your backend is deployed before adding cron jobs.");
            }
            const functions = deployClassesResult.functions ?? [];
            const cronFunction = functions.find((f) => f.name === `function-${yamlFunctionName}`);
            if (!cronFunction) {
                throw new UserError(`Function ${yamlFunctionName} not found. Please make sure the function is deployed before adding cron jobs.`);
            }
            crons.push({
                name: cron.name,
        ...
        await syncCrons({
            ...
            throw new UserError("Something went wrong while syncing the cron jobs. Please try to redeploy your project. If the problem persists, please contact support at contact@genez.io.");
        });
    }
...
export async function deployClasses(configuration, options) {
    ...
    sdkLanguages.push(...(configuration.frontend || [])
        .map((f) => f.sdk?.language)
        .filter((f) => f !== undefined));
    // Add linked frontends
    sdkLanguages.push(...(await getLinkedFrontendsForProject(configuration.name)).map((f) => f.language));
    ...
    const projectConfiguration = new ProjectConfiguration(configuration, cloudProvider, sdkResponse);
    ...
    const bundlerResult = projectConfiguration.classes.map(async (element) => {
        const ast = sdkResponse.classesInfo.find((classInfo) => classInfo.classConfiguration.path === element.path).program;
        const output = await bundle(projectConfiguration, ast, element, options.installDeps, options.disableOptimization);
    ...
    const functionsResultArray = projectConfiguration.functions.map((f) => functionToCloudInput(f, backend.path, 
    /* outputDir */ undefined, backend.language.runtime));
    ...
    const environmentVariables = await createBackendEnvVarList(options.env, options.stage, configuration);
    ...
    if (sdkResponse.generatorResponses.length > 0 &&
        sdkResponse.generatorResponses[0].files.length <= 0) {
        log.info(colors.cyan("Your backend code was successfully deployed!"));
        return;
    }
    else {
        log.info(colors.cyan(`Your backend code was ${result.classes.length > 0 ? "deployed and the SDK was successfully generated" : "successfully deployed"}`));
    }
...
export async function functionToCloudInput(functionElement, backendPath, outputDir, runtime) {
    ...
    if (!supportedFunctionLanguages.includes(functionElement.language)) {
        throw new UserError(`The language ${functionElement.language} is not supported for functions. Supported languages are: ${supportedFunctionLanguages.join(", ")}`);
    }
    const handlerProvider = getFunctionHandlerProvider(functionElement.type, functionElement.language);
    if (outputDir && !fs.existsSync(outputDir)) {
        debugLogger.debug(`Creating output directory ${outputDir}`);
        mkdirSync(outputDir, { recursive: true });
    }
    const tmpFolderPath = await createTemporaryFolder();
    const archivePath = path.join(outputDir || (await createTemporaryFolder()), `genezioDeploy.zip`);
    if (functionElement.language === Language.python ||
        functionElement.language === Language.pythonAsgi) {
        await fsExtra.copy(path.join(backendPath, functionElement.path), tmpFolderPath, {
            filter: (src) => {
                const relativePath = path.relative(path.join(backendPath, functionElement.path), src);
    ...
    if (functionElement.language === "js" || functionElement.language === "ts") {
        if (fsExtra.pathExistsSync(path.join(tmpFolderPath, "node_modules", ".pnpm"))) {
            ...
            await fsExtra.copy(path.join(backendPath, functionElement.path, "node_modules", ".pnpm"), path.join(tmpFolderPath, "node_modules", ".pnpm"), {
                dereference: true,
            });
        }
    }
    ...
    if (functionElement.language === Language.python ||
        ...
        if (fs.existsSync(requirementsPath) || fs.existsSync(pyProjectTomlPath)) {
            ...
            const localPythonVersion = (await detectPythonVersion())?.match(/\d+\.\d+/)?.[0] ||
    ...
    let entryFileName;
    if (functionElement.type === FunctionType.httpServer ||
        functionElement.type === FunctionType.persistent) {
        entryFileName = path.join(functionElement.entry).replace(/\\/g, "/");
    }
    else {
        entryFileName =
            entryFileFunctionMap[functionElement.language];
        while (fs.existsSync(path.join(tmpFolderPath, entryFileName))) {
            debugLogger.debug(`[FUNCTION ${functionElement.name}] File ${entryFileName} already exists in the temporary folder.`);
            entryFileName = getFunctionEntryFilename(functionElement.language, `index-${Math.random().toString(36).substring(7)}`);
        }
        if (handlerProvider) {
...
export async function deployFrontend(name, region, frontend, index, options, configuration) {
    ...
    await doAdaptiveLogAction(`Building frontend ${index + 1}`, async () => {
        ...
        debugLogger.debug(`Environment variables injected for frontend.scripts.build:`, JSON.stringify(newEnvObject));
    ...
    if (!(await fileExists(path.join(frontend.path, frontend.publish)))) {
        throw new UserError(`The frontend path ${colors.cyan(`${frontend.publish}`)} does not exist. Please make sure the path is correct.`);
    }
...
async function handleSdk(configuration, result, sdk, options) {
    const frontends = configuration.frontend;
    const sdkLocations = [];
