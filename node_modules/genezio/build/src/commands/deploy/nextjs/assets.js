 ...
 */
export async function computeAssetsPaths(assetsFolder, origin) {
 ...
 */
export async function treeFs(fsPath) {
    ...
    await Promise.all((await fs.promises.readdir(fsPath)).map(async (file) => {
 ...
 */
export async function mergeFsTrees(root, add) {
    if (!root && !add) {
 ...
 * @returns The paths that the assets should be served at
 */
export function recursivePathGenerator(assetsEntry, routeEntry, currentPath = "", paths = []) {
    ...
    for (const [name, entry] of assetsEntry.children) {
        ...
        const routeWildcard = [...routeEntry.children.keys()].find((e) => e.startsWith("[") && e.endsWith("]"));
        if (routeEntry.children.has(name)) {
            recursivePathGenerator(assetsEntry.children.get(name), routeEntry.children.get(name), [currentPath, name].join("/"), paths);
        }
        else if (routeWildcard) {
            recursivePathGenerator(assetsEntry.children.get(name), routeEntry.children.get(routeWildcard), [currentPath, name].join("/"), paths);
        }
        else {
            paths.push([currentPath, name, "*"].join("/"));
        }
