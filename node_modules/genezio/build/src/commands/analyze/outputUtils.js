import { SUPPORTED_FORMATS } from "./command.js";
export function report(format, frameworks, config) {
    switch (format) {
...
function formatFrameworksSimpleList(frameworks) {
...
function prettyLogGenezioConfig(config) {
    const name = config.name ? `Application Name: ${config.name}` : "";
    const region = config.region ? `Region: ${config.region}` : "";
    const component = (initialDecription, componentName, componentConfig) => {
    ...
    const components = [
        config.nextjs
            ? component("We found a Next.js component in your project:", "Next.js", config.nextjs)
            : "",
        config.nuxt
            ? component("We found a Nuxt component in your project:", "Nuxt", config.nuxt)
            : "",
        config.nitro
            ? component("We found a Nitro component in your project:", "Nitro", config.nitro)
            : "",
    ]
        .filter(Boolean)
        .join("\n\n");
...
function prettyMarkdownGenezioConfig(config) {
    ...
    const formatComponentMarkdown = (componentDescription, componentName, componentConfig) => {
        const path = componentConfig.path
            ? `- **Path to ${componentName}:** \`${componentConfig.path}\``
            : "- **Path:** Not specified";
        const packageManager = componentConfig.packageManager
    ...
    const components = [
        config.nextjs
            ? formatComponentMarkdown("This project includes a Next.js component, a server-rendered React framework for efficient page load times and SEO.", "Next.js", config.nextjs)
            : "",
        config.nuxt
            ? formatComponentMarkdown("This project includes a Nuxt component, a Vue-based framework optimized for server-side rendering and ease of deployment.", "Nuxt", config.nuxt)
            : "",
        config.nitro
            ? formatComponentMarkdown("This project includes a Nitro component, a versatile framework for server-rendered applications.", "Nitro", config.nitro)
            : "",
    ]
        .filter(Boolean)
        .join("\n\n");
