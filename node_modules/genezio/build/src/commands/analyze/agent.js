...
import { z } from "zod";
import { ENVIRONMENT_ANALYZE_PROMPT, INJECT_SERVICES, SYSTEM_ENVIRONMENT_ANALYZE_PROMPT, } from "./constants.js";
...
export async function analyzeEnvironmentVariableExampleFile(contents, services) {
    if (!process.env["OPENAI_API_KEY"]) {
        ...
        try {
            ...
            const projectEnvironments = Object.entries(parsedEnv).map(([key, value]) => ({
                key,
                defaultValue: value || "",
    ...
    try {
        ...
        const injectedServices = services
            ...
            : undefined;
        const prompt = ENVIRONMENT_ANALYZE_PROMPT.replace("{{injectedServices}}", injectedServices || "").replace("{{contents}}", contents);
